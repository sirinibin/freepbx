...
if(isset($amp_conf['SS_API_URL'])){
	$sipstation->set_api_url($amp_conf['SS_API_URL']);
}
$tabindex       = 0;
$dispnum        = "sipstation";
...
$newlook = load_view(__DIR__.'/views/header.php',$display_data);
switch ($status) { // nokey, valid, invalid, noserver
	case 'convert': //converting a trial account
		...
		$data = $sipstation->get_config($key,false);
	...
	case 'noserver':
	case 'tempnotavail':
		$newlook .= load_view(dirname(__FILE__).'/views/invalidnoservertempnot.php',$display_data);
	break;
	case 'valid':
		$key = $sipstation->get_key();
		$data = $sipstation->get_config($key,false);
		$ssobj = $data;
		if(isset($ssobj['sip_username'])){
			unset($ssobj['sip_username']);
		}
		if(isset($ssobj['sip_password'])){
			unset($ssobj['sip_password']);
		}
		echo '<script>var Sipstation = '.json_encode($ssobj).';</script>';
		...
		if($sipstation->is_private_ip($data['gateway_info'][$gw1]['contact_ip'])) {
		...
		if($sipstation->is_private_ip($data['gateway_info'][$gw2]['contact_ip'])) {
		...
		$display_data['global_failover_num'] = !empty($data['failover_num']) ? $data['failover_num'] : _('Not Set');
		$display_data['global_failover_dest'] = $data['failover_dest'];
		...
		$data['dids'] = is_array($data['dids'])?$data['dids']:array();
		...
		$newlook .= load_view(__DIR__.'/views/newlook.php',$display_data);

	break;
	case 'outdated':
		$newlook .= load_view(dirname(__FILE__).'/views/outdated.php',$display_data);
	break;
} // end switch ($status)

$sipdriver = \FreePBX::Config()->get('ASTSIPDRIVER');
$sdalert = '';
$alerttext = _("The SIPSTATION module requires that you have chan_sip enabled. Please set your SIP driver to chan_pjsip or both in Advanced settings.");
$nt = \notifications::create();
$rawname = "sipstation";
$uid = "sschandriver";
if($sipdriver == "chan_pjsip"){
	$sdalert = '<div class="alert alert-danger">'.$alerttext.'</div>';
	$sdalert .= '<script>$( document ).ready(function() { $("#localset :input").prop("disabled", true);});</script>';
	if(!$nt->exists($rawname, $uid)) {
   		 $nt->add_error($rawname, $uid, _("SIPSTATION Channel Driver"), $alerttext, "?display=advancedsettings", true, false);
	}
}else{
	if($nt->exists($rawname, $uid)) {
...
if($nokey){
	...
	echo $sdalert;
	show_view(dirname(__FILE__).'/views/maintabbed.php',array('display_data' => $display_data, 'newlook' => $newlook));
}
?>
<script>
/*
...
$(function() {
	...
	var options = {
		...
		success:    function(data) {
			if(data.status) {
				...
				var lab = "<?php echo _("UPDATES") ?>";
				noticeMessage(data.status_message, lab);
				fpbxToast(data.status_message,_("Sipstation"),"success");
			} else {
						var lab = "<?php echo _("ERROR") ?>";
				errorMessage(data.status_message, lab);
				fpbxToast(data.status_message,_("Sipstation"),"error");
			}
			}
	};
	...
	<?php
	if (isset($_REQUEST['referrer']) && $_REQUEST['referrer'] === 'freetrial') {
	?>
