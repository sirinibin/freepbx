...
if (!empty($trunk_conflict)) {
  ...
  $did_reload = $sipstation->get_or_create_dids($json_array);
  //If the trunks don't need a reload but the DIDs do, force it
  if ($did_reload) {
    //dbug("GOT BACK TRUE from get_or_create_dids()");
    $json_array['show_reload'] = 'yes';
    $json_array['did_message'] = _('DIDs without inbound routes have been detected and default routes set, you need to reload your system for them to take effect');
  } else {
    $json_array['did_message'] = '';
  }
  if ($need_reload || $did_reload) {
    needreload();
  }
